...
export async function GET(request) {
  try {
    ...
    const formattedLogs = logs.map(log => ({
      ...
      timeSinceSent: Math.floor((new Date() - new Date(log.sentAt)) / (1000 * 60 * 60)),
    }));
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to retrieve email logs. Please try again later.' },
      { status: 500 }
    );
  }
}
    ...
    switch (action) {
      case 'bulkDelete':
        ...
        if (!Array.isArray(deleteIds) || deleteIds.length === 0) {
          return NextResponse.json(
            { success: false, error: 'logIds array is required for bulkDelete.' },
            { status: 400 }
          );
        }
      ...
      case 'bulkUpdate':
        ...
        if (!Array.isArray(logIds) || logIds.length === 0) {
          return NextResponse.json(
            { success: false, error: 'logIds array is required for bulkUpdate.' },
            { status: 400 }
          );
        }
