    ...
    try {
      ...
      if (urlParams.websiteId) {
        ...
        if (targetWebsite) {
          ...
          showToast("Website not found with the provided ID", "error");
        }
      } else {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        showToast(
          error.message || "An error occurred while saving the list",
          "error"
        );
      } finally {
    ...
    (step) => {
      switch (step) {
        case STEPS.BASIC_INFO:
          if (!formData.name) {
            showToast("Please enter a name for the list.", "error");
            return false;
          }
          break;
