...
export default function PayinPage() {
  ...
  const [categoryId, setCategoryId] = useState(
    String(selected?.defaultCategoryId ?? "")
  );
  const [channelId, setChannelId] = useState(
    String(selected?.defaultChannelId ?? "")
  );
        ...
        <div className="mb-4 flex gap-2">
          <button
            ...
            className={`px-3 py-1 rounded ${
              tab === "api" ? "bg-indigo-600 text-white" : "bg-gray-200"
            }`}
          >
            Direct API
          </button>
          <button
            onClick={() => setTab("pwa")}
            className={`px-3 py-1 rounded ${
              tab === "pwa" ? "bg-indigo-600 text-white" : "bg-gray-200"
            }`}
          >
            PWA
          </button>
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                {serverToken?.access_token && (
                  <span className="text-green-700 text-sm">
                    Token ready (expires_in {serverToken.expires_in}s)
                  </span>
                )}
                {authError && (
            ...
            <section className="mb-6 rounded-xl border bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <input
                  ...
                  placeholder="CustomerTransactionId"
                  className="border p-2 rounded"
                />
                <input
                  value={itemName}
              ...
              <button
                onClick={submitPayin}
                disabled={payinLoading}
                className="mt-3 bg-emerald-600 text-white px-4 py-2 rounded flex items-center gap-2"
              >
